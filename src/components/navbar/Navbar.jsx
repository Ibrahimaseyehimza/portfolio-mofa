// import React from 'react'

// export default function Navbar() {
//     const listNavbar = [
//         {name: 'Acceuil', link:'#home'},
//         {name: 'Compétences', link:'#skills'},
//         {name: 'Experience', link:'#experience'},
//         {name: 'Language', link:'#language'},
//         {name: 'Projects', link:'#projects'},
//     ];
//   return (
//     <header data-aos='fade-up' className='text-gray-600 body-font z-10'>
//         <div className='container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center'>
//             <a className='flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0'>
//                 {/* <span className='ml-3 text-3xl font-bold text-white '>Portfolio</span> */}
//                 <span className='ml-3 text-3xl font-bold text-white '>Mofa Designer</span>
//             </a>
//             <nav className='md:ml-auto text-white text-base md:mr-auto flex flex-wrap items-center justify-center'>
//                 {
//                     listNavbar.map((item, index) =>(
//                         <a key={index} href={item.link} className='mr-5 hover:text-yellow-300'>
//                             {item.name}
//                         </a>
//                     ))
//                 }
//             </nav>
//             <button className='inline-flex items-center py-1 px-7 focus:outline-none text-base text-white mt-4
//              md:mt-0 border border-white hover:border-yellow-300 hover:text-gray-900'>
//                 <a href='#contact'>contactez-moi</a>
//             </button>
//         </div>
//     </header>
//   )
// }



import React, { useState } from 'react'

export default function Navbar() {

const [open, setOpen] = useState(false)

const listNavbar = [
    {name: 'Acceuil', link:'#home'},
    {name: 'Compétences', link:'#skills'},
    {name: 'Experience', link:'#experience'},
    {name: 'Language', link:'#language'},
    {name: 'Projects', link:'#projects'},
]

return (
<header className="bg-black fixed w-full z-50">

<div className="container mx-auto px-6 py-4 flex justify-between items-center">

{/* Logo */}
<h1 className="text-white text-2xl font-bold">
Mofa Designer
</h1>

{/* Hamburger */}
<div className="md:hidden">
<button onClick={() => setOpen(!open)} className="text-white text-3xl">
☰
</button>
</div>

{/* Desktop menu */}
<nav className="hidden md:flex space-x-6 text-white">
{listNavbar.map((item,index)=>(
<a key={index} href={item.link} className="hover:text-yellow-300">
{item.name}
</a>
))}
<a href="#contact" className="border px-5 py-1 hover:border-yellow-300">
Contactez-moi
</a>
</nav>

</div>

{/* Mobile menu */}
{open && (
<div className="md:hidden bg-black text-white flex flex-col items-center space-y-4 py-6">

{listNavbar.map((item,index)=>(
<a
key={index}
href={item.link}
onClick={()=>setOpen(false)}
className="hover:text-yellow-300"
>
{item.name}
</a>
))}

<a href="#contact" className="border px-6 py-2">
Contactez-moi
</a>

</div>
)}

</header>
)
}
